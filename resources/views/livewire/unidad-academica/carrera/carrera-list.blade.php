<div>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
            <thead class="bg-gray-50 dark:bg-gray-800">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-300">Facultad</th>
                    <th class="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-300">Departamento</th>
                    <th class="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-300">Centros donde se imparte</th>
            ...
            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-100 dark:divide-gray-800">
                @forelse ($records as $record)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors {{ $record->trashed() ? 'opacity-60' : '' }}">
                        ...
                        <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ $record->facultadcentro?->nombre }}</td>
                        <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ $record->departamentoAcademico?->nombre }}</td>
                        <td class="px-4 py-3">
                            <div class="flex flex-wrap gap-1">
                                @foreach($record->facultadCentros as $fc)
                                    <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300">{{ $fc->siglas ?: $fc->nombre }}</span>
                                @endforeach
                            </div>
                        </td>
                        <td class="px-4 py-3 text-right space-x-2">
                            @if($record->trashed())
                                <button type="button" x-on:click.prevent="confirmDialog('¿Restaurar esta carrera?').then((ok) => ok && $wire.restore({{ $record->id }}))"
                            ...
                            @else
                                ...
                                <button type="button" x-on:click.prevent="confirmDialog('¿Eliminar esta carrera?', { type: 'danger' }).then((ok) => ok && $wire.delete({{ $record->id }}))"
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-6 text-center text-gray-500 dark:text-gray-400">No se encontraron registros.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if ($createModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" wire:click.self="$set('createModal', false)">
            <div class="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-2xl p-6 mx-4 max-h-[90vh] overflow-y-auto">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Nueva Carrera</h3>
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Facultad principal</label>
                        <select wire:model="create_facultad_centro_id"
                            class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm">
                    ...
                    <div class="col-span-2">
                        ...
                        <select wire:model="create_departamento_academico_id"
                            class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm">
                            <option value="">Seleccione un departamento</option>
                            @foreach($departamentosAcad as $id => $nombre)
                                <option value="{{ $id }}">{{ $nombre }}</option>
                            @endforeach
                        </select>
                        @error('create_departamento_academico_id') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div class="col-span-2">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Centros donde se imparte</label>
                        <div class="grid grid-cols-2 gap-1 max-h-40 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-md p-2">
                            @foreach($centros as $id => $nombre)
                                <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                                    <input type="checkbox" wire:model="create_facultad_centros" value="{{ $id }}" class="rounded border-gray-300">
                                    {{ $nombre }}
                                </label>
                            @endforeach
                        </div>
    ...
    @if ($editModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" wire:click.self="$set('editModal', false)">
            <div class="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-2xl p-6 mx-4 max-h-[90vh] overflow-y-auto">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Editar Carrera</h3>
                <div class="grid grid-cols-2 gap-4">
                    <div class="col-span-2">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nombre *</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Facultad principal</label>
                        <select wire:model="edit_facultad_centro_id"
                            class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm">
                            <option value="">Ninguna</option>
                            @foreach($facultades as $id => $nombre)
                                <option value="{{ $id }}">{{ $nombre }}</option>
                    ...
                    <div class="col-span-2">
                        ...
                        @error('edit_departamento_academico_id') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div class="col-span-2">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Centros donde se imparte</label>
                        <div class="grid grid-cols-2 gap-1 max-h-40 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-md p-2">
                            @foreach($centros as $id => $nombre)
                                <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                                    <input type="checkbox" wire:model="edit_facultad_centros" value="{{ $id }}" class="rounded border-gray-300">
                                    {{ $nombre }}
                                </label>
                            @endforeach
                        </div>
