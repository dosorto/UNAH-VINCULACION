                ...
                @forelse ($records as $record)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors {{ $record->trashed() ? 'opacity-60' : '' }}">
                        ...
                        <td class="px-4 py-3 text-right space-x-2">
                            @if($record->trashed())
                                <button x-on:click.prevent="confirmDialog('¿Restaurar esta carrera?').then((ok) => ok && $wire.restore({{ $record->id }}))"
                            ...
                            @else
                                ...
                                <button x-on:click.prevent="confirmDialog('¿Eliminar esta carrera?', { type: 'danger' }).then((ok) => ok && $wire.delete({{ $record->id }}))"
