<div>
    ...
    <div class="mb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
            <p class="text-zinc-950 dark:text-white font-bold mb-1">Jornadas Laborales</p>
            <p class="text-zinc-500 dark:text-gray-400 font-medium text-sm">Rangos de horario disponibles para seleccionar como jornada laboral del docente supervisor.</p>
        </div>
    ...
    <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
            <thead class="bg-gray-50 dark:bg-gray-800">
                <tr>
                    <th class="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-300">Jornada</th>
            ...
            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-100 dark:divide-gray-800">
                @forelse ($records as $record)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors {{ $record->trashed() ? 'opacity-60' : '' }}">
                        <td class="px-4 py-3 text-gray-900 dark:text-white font-medium">{{ $record->etiqueta }}</td>
                        ...
                        <td class="px-4 py-3 text-right space-x-2">
                            @if($record->trashed())
                                <button type="button" x-on:click.prevent="confirmDialog('¿Restaurar esta jornada laboral?').then((ok) => ok && $wire.restore({{ $record->id }}))"
                            ...
                            @else
                                ...
                                <button type="button" x-on:click.prevent="confirmDialog('¿Eliminar esta jornada laboral?', { type: 'danger' }).then((ok) => ok && $wire.delete({{ $record->id }}))"
    ...
    @if ($createModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" wire:click.self="$set('createModal', false)">
            <div class="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-md p-6 mx-4">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">Nueva Jornada Laboral</h3>
                <p class="text-xs text-gray-500 dark:text-gray-400 mb-4">Formato de 24 horas (Ej: 07:00, 13:00, 19:30).</p>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Hora inicio *</label>
                        <input type="time" lang="en-GB" wire:model="create_hora_inicio"
                            class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm" />
                        @error('create_hora_inicio') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Hora fin *</label>
                        <input type="time" lang="en-GB" wire:model="create_hora_fin"
                            class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm" />
                        @error('create_hora_fin') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
    ...
    @if ($editModal)
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" wire:click.self="$set('editModal', false)">
            <div class="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-md p-6 mx-4">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">Editar Jornada Laboral</h3>
                <p class="text-xs text-gray-500 dark:text-gray-400 mb-4">Formato de 24 horas (Ej: 07:00, 13:00, 19:30).</p>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Hora inicio *</label>
                        <input type="time" lang="en-GB" wire:model="edit_hora_inicio"
                            class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm" />
                        @error('edit_hora_inicio') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Hora fin *</label>
                        <input type="time" lang="en-GB" wire:model="edit_hora_fin"
                            class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm" />
                        @error('edit_hora_fin') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Orden</label>
