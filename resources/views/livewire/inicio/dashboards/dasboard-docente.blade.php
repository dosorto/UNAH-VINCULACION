                                ...
                                <tbody>
                                    @forelse($misFormularios as $formulario)
                                        <tr
                                                ...
                                                @if (! empty($formulario['stepper']))
                                                    @php
                                                        $faseClase = match ($formulario['fase'] ?? 'Aprobación') {
                                                            'Informe Intermedio' => 'bg-sky-100 text-sky-700 dark:bg-sky-900 dark:text-sky-200',
                                                            'Informe Final' => 'bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-200',
                                                            default => 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300',
                                                        };
                                                    @endphp
                                                    <span class="inline-block mb-1.5 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide rounded-full {{ $faseClase }}">
                                            ...
                                            <td class="px-6 py-4">
                                                {{ $formulario['fecha_inicio'] ? \Carbon\Carbon::parse($formulario['fecha_inicio'])->isoFormat('D [de] MMMM YYYY') : '-' }}
                                            </td>
                                            <td class="px-6 py-4">
                                                {{ $formulario['fecha_fin'] ? \Carbon\Carbon::parse($formulario['fecha_fin'])->isoFormat('D [de] MMMM YYYY') : '-' }}
                                            </td>
                                            <td class="px-6 py-4">
                                                {{ $formulario['categoria'] ?: 'Sin categoría' }}
                                            </td>
                                        </tr>
                                    @empty
                                        <tr
