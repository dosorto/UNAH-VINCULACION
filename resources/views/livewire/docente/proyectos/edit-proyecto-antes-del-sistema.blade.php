<div>
    ...
    <div class="mb-6 flex items-start justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Completar Información del Proyecto</h2>
    ...
    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
        <div class="flex">
            <svg class="h-5 w-5 text-blue-400 shrink-0 mt-0.5" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" />
            </svg>
            <div class="ml-3 text-sm text-blue-700 dark:text-blue-300">
                <p>Complete la información faltante y guarde. Al completar, el proyecto cambiará a estado <strong>Finalizado</strong>.</p>
            </div>
        </div>
    </div>
    ...
    <div class="mb-6 bg-white dark:bg-gray-900 shadow rounded-lg p-4">
        @php $stepLabels = ['Info General','Equipo','Contraparte','Actividades','Descripción','Marco Lógico','Presupuesto','Anexos']; @endphp
    ...
    <div class="bg-white dark:bg-gray-900 shadow rounded-lg p-6">
        ...
        <div class="space-y-4">
            ...
            @endif
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Programa al que Pertenece</label>
                <input type="text" wire:model="programa_pertenece" class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm text-gray-900 dark:text-white focus:border-blue-500" />
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Líneas de Investigación Académica</label>
        ...
        <div class="space-y-6">
            <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
                ...
                <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ auth()->user()->empleado?->nombre_completo ?? auth()->user()->name }}</p>
        ...
        <div class="space-y-4">
            ...
            @endforeach
            <button wire:click="addContraparte" type="button" class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-md bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200">
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <textarea wire:model="alineamiento_reforma" rows="2" class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm focus:border-blue-500"></textarea>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Impacto Deseado</label>
                <textarea wire:model="impacto_deseado" rows="2" class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm focus:border-blue-500"></textarea>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Metodología</label>
                <textarea wire:model="metodologia" rows="3" class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm focus:border-blue-500"></textarea>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Bibliografía</label>
                <textarea wire:model="bibliografia" rows="2" class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm focus:border-blue-500"></textarea>
            </div>
        </div>
        @endif
        ...
        <div class="space-y-4">
            ...
            <div>
                <div class="flex items-center justify-between mb-3">
                    <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300">Objetivos Específicos</h4>
                    <button wire:click="addObjetivo" type="button" class="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-md bg-blue-600 text-white hover:bg-blue-700">+ Agregar</button>
                </div>
                @foreach($objetivosEspecificos as $oi => $objetivo)
                <div class="mb-4 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                    <div class="flex items-center justify-between mb-2">
                        <h5 class="text-sm font-semibold text-gray-700 dark:text-gray-300">Objetivo {{ $oi + 1 }}</h5>
                    ...
                    <div class="ml-4">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <button wire:click="addResultado({{ $oi }})" type="button" class="text-xs text-blue-600 hover:text-blue-800">+ Agregar</button>
                        </div>
                        ...
                        <div class="mb-2 p-3 bg-gray-50 dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-600">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                <div><label class="block text-xs font-medium text-gray-500 mb-1">Resultado</label><input type="text" wire:model="objetivosEspecificos.{{ $oi }}.resultados.{{ $ri }}.nombre_resultado" class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-2 py-1 text-xs focus:border-blue-500" /></div>
                                <div><label class="block text-xs font-medium text-gray-500 mb-1">Indicador</label><input type="text" wire:model="objetivosEspecificos.{{ $oi }}.resultados.{{ $ri }}.nombre_indicador" class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-2 py-1 text-xs focus:border-blue-500" /></div>
                                <div><label class="block text-xs font-medium text-gray-500 mb-1">Medio de Verificación</label><input type="text" wire:model="objetivosEspecificos.{{ $oi }}.resultados.{{ $ri }}.nombre_medio_verificacion" class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-2 py-1 text-xs focus:border-blue-500" /></div>
                                <div><label class="block text-xs font-medium text-gray-500 mb-1">Plazo</label><input type="text" wire:model="objetivosEspecificos.{{ $oi }}.resultados.{{ $ri }}.plazo" class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-2 py-1 text-xs focus:border-blue-500" /></div>
                            </div>
                            <div class="mt-1 text-right"><button wire:click="removeResultado({{ $oi }}, {{ $ri }})" type="button" class="text-xs text-red-600 hover:text-red-800">Eliminar</button></div>
                        </div>
                        @endforeach
                    </div>
                </div>
        ...
        <div class="space-y-6">
            <div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                        <thead class="bg-gray-50 dark:bg-gray-800 text-xs text-gray-500 text-left">
                            <tr><th class="py-2 px-3">Concepto</th><th class="py-2 px-3">Unidad</th><th class="py-2 px-3 text-center">Cantidad</th><th class="py-2 px-3 text-center">Costo Unitario</th><th class="py-2 px-3 text-center">Costo Total</th></tr>
                        </thead>
                        <tbody>
                            @foreach($aporte_institucional as $i => $aporte)
                            <tr class="border-t border-gray-200 dark:border-gray-700">
                                <td class="py-2 px-3 text-xs text-gray-700 dark:text-gray-300">{{ $aporte['concepto_label'] ?? $aporte['concepto'] }}</td>
                                <td class="py-2 px-3 text-xs text-gray-500">{{ $aporte['unidad_label'] ?? $aporte['unidad'] }}</td>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Aporte Contraparte</label><input type="number" wire:model="aporte_contraparte" min="0" step="0.01" class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm focus:border-blue-500" /></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Aporte Internacionales</label><input type="number" wire:model="aporte_internacionales" min="0" step="0.01" class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm focus:border-blue-500" /></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Aporte Otras Universidades</label><input type="number" wire:model="aporte_otras_universidades" min="0" step="0.01" class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm focus:border-blue-500" /></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Aporte Comunidad</label><input type="number" wire:model="aporte_comunidad" min="0" step="0.01" class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm focus:border-blue-500" /></div>
                    <div><label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Otros Aportes</label><input type="number" wire:model="otros_aportes" min="0" step="0.01" class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm focus:border-blue-500" /></div>
                </div>
            </div>
        </div>
        @endif
        ...
        <div class="space-y-4">
            <div class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6">
                <div class="space-y-3">
                    ...
                    <button wire:click="uploadAnexo" type="button" class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-md bg-blue-600 text-white hover:bg-blue-700">Subir Anexo</button>
                </div>
            </div>
            @php $anexos = $record->anexos ?? collect(); @endphp
            @if($anexos->count())
            <div>
                ...
                <div class="flex items-center justify-between py-2 px-3 border border-gray-200 dark:border-gray-700 rounded-lg mb-2">
                    ...
                    <button type="button" x-on:click.prevent="confirmDialog('¿Eliminar este anexo?', { type: 'danger' }).then((ok) => ok && $wire.deleteAnexo({{ $anexo->id }}))" type="button" class="text-xs text-red-600 hover:text-red-800 ml-3 shrink-0">Eliminar</button>
                </div>
                @endforeach
            </div>
            @endif
        ...
        <div class="mt-8 flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
            <div>
                ...
                @else
                <a href="{{ route('proyectosAntesDelSistema') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50">
            ...
            <div>
                ...
                @else
                <button wire:click="save" type="button" class="inline-flex items-center px-4 py-2 rounded-md text-sm font-medium text-white bg-green-600 hover:bg-green-700">
                    Guardar Proyecto
                </button>
                @endif
            </div>
