...
<div class="text-gray-900 dark:text-gray-100">
    <header class="mb-4 px-1">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <p class="text-xs font-semibold uppercase tracking-wide text-blue-700 dark:text-blue-300">FORM-DVUS-018</p>
                <h1 class="mt-1 text-xl font-bold">Informe final de proyecto de Educacion No Formal</h1>
                ...
                <p class="mt-2 max-w-3xl text-sm text-gray-600 dark:text-gray-400">Los datos base se precargan desde el FORM-DVUS-018. Complete la informacion real de cierre, ejecucion, evaluacion y sistematizacion.</p>
            </div>
    ...
    <main class="rounded-lg bg-white p-6 shadow dark:bg-gray-900">
        @if($currentStep === 1)
            <h2 class="mb-6 text-lg font-semibold">Paso 1: Informacion general del proyecto ENF</h2>
            <p class="-mt-4 mb-5 text-xs text-gray-500"><span class="font-medium text-blue-600">Datos precargados.</span> Estos campos vienen del FORM-DVUS-018 y sirven como base del cierre.</p>
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div><label class="{{ $label }}">Numero de registro</label><input value="{{ $accion->numero_registro ?: 'Pendiente' }}" class="{{ $readonly }}" readonly></div>
                ...
                <div><label class="{{ $label }}">Tipo de accion ENF</label><input value="{{ $accion->tipoAccion?->nombre ?: 'Pendiente' }}" class="{{ $readonly }}" readonly></div>
                <div><label class="{{ $label }}">Modalidad</label><input value="{{ $accion->modalidad?->nombre ?: ($lugar?->modalidad_ejecucion ?: 'Pendiente') }}" class="{{ $readonly }}" readonly></div>
                ...
                <div><label class="{{ $label }}">Lugar / sede</label><input value="{{ collect([$lugar?->nombre_lugar, $lugar?->municipio?->nombre, $lugar?->departamento?->nombre])->filter()->implode(' / ') ?: 'Pendiente' }}" class="{{ $readonly }}" readonly></div>
        ...
        @elseif($currentStep === 2)
            <h2 class="mb-6 text-lg font-semibold">Paso 2: Equipo ejecutor y coordinacion</h2>
            <div class="grid gap-4 md:grid-cols-2">
            ...
            <h3 class="mt-7 font-semibold">Equipo ejecutor precargado</h3>
        ...
        @elseif($currentStep === 3)
            <div class="flex items-center justify-between gap-3">
                ...
                <button type="button" wire:click="agregarFila('participantes')" class="{{ $button }} bg-blue-50 text-blue-700">Agregar beneficiario/participante</button>
            </div>
            <div class="mt-5 overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr><td colspan="8" class="p-3 text-gray-500">Agregue los participantes finales del proyecto ENF.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        @elseif($currentStep === 4)
            <h2 class="mb-6 text-lg font-semibold">Paso 4: Resumen estadistico de beneficiarios</h2>
            <div class="mb-5 grid gap-3 sm:grid-cols-3">
                <div class="rounded bg-gray-50 p-3 text-sm dark:bg-gray-800"><strong>Programados FORM 18</strong><p>Hombres: {{ $this->beneficiariosProgramados['hombres'] }}</p><p>Mujeres: {{ $this->beneficiariosProgramados['mujeres'] }}</p><p>Total: {{ $this->beneficiariosProgramados['total'] }}</p></div>
        ...
        @elseif($currentStep === 5)
            <h2 class="mb-6 text-lg font-semibold">Paso 5: Contrapartes, participacion y enfoque</h2>
            <div class="grid gap-4 md:grid-cols-2">
                <div><label class="{{ $label }}">Tipo de accion ejecutada</label><input wire:model.live.debounce.600ms="general.modalidad_acreditacion" class="{{ $input }}"></div>
                <div><label class="{{ $label }}">Participacion universitaria</label><input value="{{ $accion->participacionUniversitaria->pluck('tipo_participacion')->filter()->implode(', ') ?: 'No registrada' }}" class="{{ $readonly }}" readonly></div>
                <div class="md:col-span-2"><label class="{{ $label }}">Ajustes metodologicos o de alcance</label><textarea rows="3" wire:model.live.debounce.1000ms="general.contenido_curricular_cambios" class="{{ $input }}"></textarea></div>
            ...
            <h3 class="mt-7 font-semibold">Contrapartes registradas</h3>
            <div class="mt-3 overflow-x-auto">
                <table class="min-w-full text-sm"><thead class="bg-gray-50 dark:bg-gray-800"><tr><th class="px-3 py-2 text-left">Institucion</th><th class="px-3 py-2 text-left">Tipo</th><th class="px-3 py-2 text-left">Aporte / instrumento</th></tr></thead><tbody>@forelse($accion->contrapartes as $contraparte)<tr class="border-t dark:border-gray-700"><td class="px-3 py-2">{{ $contraparte->nombre }}</td><td class="px-3 py-2">{{ $contraparte->tipoContraparte?->nombre ?: '-' }}</td><td class="px-3 py-2">{{ $contraparte->instrumentoAlianza?->nombre ?: ($contraparte->aporte ?: '-') }}</td></tr>@empty<tr><td colspan="3" class="p-3 text-gray-500">Sin contrapartes registradas.</td></tr>@endforelse</tbody></table>
            </div>
        @elseif($currentStep === 6)
            <h2 class="mb-6 text-lg font-semibold">Paso 6: Ejecucion, resultados y sistematizacion</h2>
        ...
        @elseif($currentStep === 7)
            <h2 class="mb-6 text-lg font-semibold">Paso 7: Evaluacion, presupuesto e impacto</h2>
            ...
            <h3 class="mt-7 font-semibold">Presupuesto precargado desde FORM-DVUS-018</h3>
        ...
        @else
            ...
            <section class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-xs text-gray-500">Respaldos, listados, actas, evidencias, informes, bitacoras y documentos cargados desde el FORM-DVUS-018.</p>
                    </div>
            ...
            <section class="mt-6 rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                <h3 class="mb-4 font-semibold">Fotografias del proyecto ENF</h3>
                <x-forms.image-dropzone model="fotografiasTemporales" id="form018-fotografias" />
                @if($this->fotografias)
            ...
            <section class="mt-6 rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                ...
                <div class="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    <div class="rounded-md bg-gray-50 p-3 dark:bg-gray-800"><p class="text-xs text-gray-500">Contrapartes</p><p class="mt-1 font-semibold">{{ $accion->contrapartes->count() }}</p></div>
                    <div class="rounded-md bg-gray-50 p-3 dark:bg-gray-800"><p class="text-xs text-gray-500">Equipo ejecutor</p><p class="mt-1 font-semibold">{{ $accion->equipo->count() }}</p></div>
                    ...
                    <div class="rounded-md bg-gray-50 p-3 dark:bg-gray-800"><p class="text-xs text-gray-500">Actividades ejecutadas</p><p class="mt-1 font-semibold">{{ collect($accionesEjecutadas)->filter(fn ($row) => filled($row['actividad'] ?? null))->count() }}</p></div>
                </div>
            </section>
            ...
            <section class="mt-6 rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                ...
                <div class="mt-3 grid gap-4 sm:grid-cols-2">
                    ...
                    <label class="flex items-start gap-2"><input type="checkbox" wire:model.live="general.confirmacion_veracidad" class="mt-1 rounded border-gray-300 text-blue-600"><span class="text-sm">Confirmo que la informacion consignada en el informe final FORM-DVUS-018 es veraz.</span></label>
                </div>
            </section>
        ...
        <div class="mt-8 flex flex-col gap-3 border-t border-gray-200 pt-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap items-center justify-end gap-3">
                ...
                @else
                    ...
                    <button type="button" x-on:click.prevent="confirmDialog('Confirma que desea marcar completo el informe final FORM-DVUS-018?').then((ok) => ok && $wire.validarInforme())"  class="{{ $button }} bg-green-700 text-white">Validar informe</button>
