<div class="space-y-8">
    <section class="flex flex-col gap-4 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800 md:flex-row md:items-end md:justify-between">
        <div>
            ...
            <h1 class="mt-2 text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Catalogos nucleo DAFT</h1>
    ...
    <div class="grid gap-8 xl:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)]">
        <section class="space-y-6">
            <article class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
                ...
                <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Gestiona las bases de campus y centros para DAFT.</p>
                <div class="mt-5 grid gap-3 md:grid-cols-2">
                    <a href="{{ route('campus') }}" class="rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-primary/40 hover:bg-primary/5 dark:border-slate-800 dark:text-slate-200 dark:hover:bg-slate-800/60">
                        Campus
                    </a>
                    <a href="{{ route('facultad-centro') }}" class="rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-primary/40 hover:bg-primary/5 dark:border-slate-800 dark:text-slate-200 dark:hover:bg-slate-800/60">
        ...
        <section class="space-y-6">
            <article class="rounded-3xl bg-gradient-to-br from-primary to-primary-container p-6 text-white shadow-lg">
                <h2 class="text-lg font-semibold">Catalogos de programas</h2>
                <p class="mt-3 text-sm text-on-primary-container">Con estas bases listas puedes construir programas, agregar asignaturas, definir centros y ejecutar flujos de revision.</p>
                <div class="mt-5 flex flex-wrap gap-3">
                    <a href="{{ route('daft.programas') }}" class="rounded-full bg-white px-4 py-2 text-sm font-semibold text-primary">Ir a programas</a>
                    <a href="{{ route('configuracion.flujos.proyectos') }}" class="rounded-full border border-white/20 px-4 py-2 text-sm font-semibold text-white">Ir a flujos</a>
                </div>
            </article>
        </section>
    </div>
