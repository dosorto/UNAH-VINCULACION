        ...
        <aside class="space-y-5 xl:sticky xl:top-20">
            <section class="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
                ...
                @elseif ($canAct)
                    ...
                    <div class="mt-5 space-y-3">
                        <button type="button" x-on:click.prevent="confirmDialog('¿Confirmas que revisaste el expediente y deseas aprobar esta etapa?').then((ok) => ok && $wire.approveRevision())"  class="inline-flex w-full items-center justify-center gap-2 rounded-full bg-emerald-600 px-4 py-3 text-sm font-bold text-white transition hover:bg-emerald-700">@svg('heroicon-o-check-badge', ['class' => 'h-5 w-5']) Aceptar y firmar etapa</button>
                        <button type="button" x-on:click.prevent="confirmDialog('¿Enviar el programa a subsanación con estas observaciones?').then((ok) => ok && $wire.rejectRevision())"  class="inline-flex w-full items-center justify-center gap-2 rounded-full border border-rose-300 px-4 py-3 text-sm font-bold text-rose-700 transition hover:bg-rose-50 dark:border-rose-800 dark:text-rose-300 dark:hover:bg-rose-950/30">@svg('heroicon-o-arrow-uturn-left', ['class' => 'h-5 w-5']) Enviar a subsanación</button>
                    </div>
                @else
