    ...
    <main class="bg-white dark:bg-gray-900 shadow rounded-lg p-5">
        @if($currentStep === 1)
            ...
            @php
                ...
                $precargados = [
                    'Nombre del programa o proyecto' => $general['nombre_proyecto'] ?? null,
                    'Número de registro' => $general['numero_registro'] ?: 'Pendiente de asignación',
                    'Fecha de registro' => $fechaRegistro,
                    'Facultad / Centro' => $general['facultad_centro'] ?? null,
                    'Unidad académica' => $general['unidad_academica'] ?? null,
                    'Departamento académico' => $general['departamento_academico'] ?? null,
                    ...
                    'Modalidad' => $general['modalidad'] ?? null,
                    'Ejes prioritarios' => $general['ejes_prioritarios'] ?? null,
                    'Categoría' => $general['categoria'] ?? null,
                    'Duración estimada' => trim(($informe->duracion_semanas ?? 0).' semanas'),
                ];
            @endphp
            ...
            <h3 class="mt-6 font-semibold">Territorio de ejecución</h3>
            ...
            <div class="mt-3 grid gap-3 sm:grid-cols-3">
                <div><label class="{{ $label }}">Región</label><input type="text" wire:model.live.debounce.1000ms="general.region" class="{{ $input }}"></div>
                <div><label class="{{ $label }}">Aldea o ciudad</label><input type="text" wire:model.live.debounce.1000ms="general.aldea_ciudad" class="{{ $input }}"></div>
                <div><label class="{{ $label }}">Caserío</label><input type="text" wire:model.live.debounce.1000ms="general.caserio" class="{{ $input }}"></div>
            </div>
            <h3 class="mt-6 font-semibold">Beneficiarios directos</h3>
            ...
            <div class="mt-2 overflow-x-auto"><table class="min-w-full text-sm"><thead class="bg-gray-50 dark:bg-gray-800"><tr>@foreach(['Nombre','Pasaporte','Correo','País','Universidad','Horas','Estado','Acciones'] as $h)<th class="px-3 py-2 text-left">{{ $h }}</th>@endforeach</tr></thead><tbody>@forelse($cooperacion as $i=>$row)@php($cActivo = ($row['estado_participacion'] ?? 'activo') === 'activo')<tr class="border-t dark:border-gray-700 {{ $cActivo ? '' : 'opacity-60' }}">
                ...
                <td class="px-3 py-2 whitespace-nowrap">
                    <div class="flex flex-wrap gap-2">
                        ...
                        @else
                            <button type="button" wire:click="restaurarParticipante('cooperacion',{{ $i }})" wire:confirm="¿Restaurar participación?" class="text-sm text-green-700">Restaurar</button>
                        @endif
                        <button type="button" wire:click="quitarFila('cooperacion',{{ $i }})" wire:confirm="¿Eliminar este integrante de cooperación internacional?" class="text-sm text-red-600">Borrar</button>
                    </div>
                </td>
        ...
        @elseif($currentStep === 7)
            ...
            <div class="mt-6 flex items-center justify-between"><h3 class="font-semibold">Detalle presupuestario</h3><button type="button" wire:click="agregarFila('presupuesto')" class="{{ $button }} bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-200">Agregar concepto</button></div>
            <div class="mt-3 overflow-x-auto"><table class="min-w-full text-sm"><thead class="bg-gray-50 dark:bg-gray-800"><tr>@foreach(['Fuente','Concepto','Unidad','Cantidad','Costo unitario','Costo total','Origen',''] as $h)<th class="px-3 py-2 text-left">{{ $h }}</th>@endforeach</tr></thead><tbody>@foreach($presupuesto as $i=>$row)@php($protegida = filled($row['id'] ?? null) || ($row['origen_fondos'] ?? null) === 'contrapartes_proyecto')<tr class="border-t dark:border-gray-700"><td><select wire:model.live="presupuesto.{{ $i }}.fuente" @disabled($protegida) class="{{ $protegida ? $readonly : $input }}"><option>UNAH</option><option>CONTRAPARTE</option></select></td><td><input wire:model="presupuesto.{{ $i }}.concepto" @readonly($protegida) class="{{ $protegida ? $readonly : $input }} min-w-36"></td><td><input wire:model="presupuesto.{{ $i }}.unidad" @readonly($protegida) class="{{ $protegida ? $readonly : $input }} min-w-24"></td><td><input type="number" min="0" step="0.01" wire:model.live="presupuesto.{{ $i }}.cantidad" @readonly($protegida) class="{{ $protegida ? $readonly : $input }} w-28"></td><td><input type="number" min="0" step="0.01" wire:model.live="presupuesto.{{ $i }}.costo_unitario" @readonly($protegida) class="{{ $protegida ? $readonly : $input }} w-32"></td><td class="px-3 py-2 font-medium">L {{ number_format((float)($row['cantidad']??0)*(float)($row['costo_unitario']??0),2) }}</td><td><input wire:model="presupuesto.{{ $i }}.origen_fondos" @readonly($protegida) class="{{ $protegida ? $readonly : $input }} min-w-32"></td><td>@unless($protegida)<button type="button" wire:click="quitarFila('presupuesto',{{ $i }})" class="text-red-600">×</button>@endunless</td></tr>@endforeach</tbody></table></div>
            <div class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div><label class="{{ $label }}">Presupuesto planificado</label><input type="number" min="0" step="0.01" wire:model.live="general.presupuesto_planificado" class="{{ $input }}"></div><div><label class="{{ $label }}">Aporte beneficiarios</label><input type="number" min="0" step="0.01" wire:model.live="general.aporte_beneficiarios" class="{{ $input }}"></div><div><label class="{{ $label }}">Otros aportes</label><input type="number" min="0" step="0.01" wire:model.live="general.otros_aportes" class="{{ $input }}"></div></div>
            <div class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">@foreach(['subtotal'=>'Subtotal base UNAH','infraestructura'=>'Infraestructura 3%','servicios'=>'Servicios públicos 3%','unah'=>'Total institucional','contraparte'=>'Aporte contraparte','ejecucion'=>'Ejecución total','porcentaje'=>'Porcentaje de ejecución'] as $field=>$name)<div class="rounded-md bg-gray-50 p-3 dark:bg-gray-800"><p class="text-xs text-gray-500">{{ $name }}</p><p class="mt-1 font-semibold">{{ $field==='porcentaje' ? number_format($this->totalesPresupuesto[$field],2).'%' : 'L '.number_format($this->totalesPresupuesto[$field],2) }}</p></div>@endforeach</div>
        @else
                            ...
                            <div class="grid grid-cols-1 items-start gap-4 md:grid-cols-2 xl:grid-cols-8">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <select
                                        ...
                                        class="{{ $esAnexoPlanificado ? $readonly : $input }}"
                                    >
