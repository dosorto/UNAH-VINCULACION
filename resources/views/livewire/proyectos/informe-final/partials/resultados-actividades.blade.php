    ...
    <div class="mt-3 w-full overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
        <table class="w-full min-w-[1100px] text-sm">
            <thead class="bg-gray-50 text-left align-bottom dark:bg-gray-800">
                <tr>@foreach([$grupo['col1'],'Resultado planificado','Indicador propuesto','Unidad','Meta','Alcanzado *','Cumpl. %','Estado','Producto logrado *','Observaciones'] as $heading)<th class="px-3 py-2 font-semibold">{{ $heading }}</th>@endforeach</tr>
            </thead>
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                @forelse($grupo['rows'] as $row)
                    ...
                    <tr wire:key="resultado-{{ $row['id'] ?? 'nuevo-'.$i }}" class="align-top">
                        ...
                        <td class="px-3 py-2"><input type="number" min="0" step="0.01" wire:model.blur.number="resultados.{{ $i }}.valor_alcanzado" class="{{ $input }} w-24">@error('resultados.'.$i.'.valor_alcanzado')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</td>
