<div>
    ...
    <div class="bg-white dark:bg-gray-900 shadow rounded-lg p-6">
        ...
        <div class="space-y-4">
            ...
            @if($metasList->count())
            <div wire:key="metas-{{ implode('-', array_map('intval', $this->ods)) }}" class="bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-800 rounded-lg p-4">
                <label class="block text-sm font-medium text-green-800 dark:text-green-300 mb-2">Metas que Contribuye (ODS seleccionados)</label>
                <div x-data="{open:false,selected:$wire.entangle('metasContribuye'),options:@js($metasList),toggle(id){id=String(id);let c=(this.selected||[]).map(String);const i=c.indexOf(id);i===-1?c.push(id):c.splice(i,1);this.selected=c;},isSelected(id){return(this.selected||[]).map(String).includes(String(id));},getName(id){return this.options[id]??this.options[String(id)]??id;}}" @click.outside="open=false" class="relative">
                    <div @click="open=!open" class="min-h-[42px] w-full rounded-md border border-green-300 dark:border-green-700 bg-white dark:bg-gray-800 px-3 py-2 cursor-pointer flex flex-wrap gap-1 items-center">
                        <template x-for="id in (selected||[])" :key="id"><span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800"><span x-text="getName(id)"></span><button type="button" @click.stop="toggle(id)" class="font-bold">×</button></span></template>
                        <span x-show="!selected||selected.length===0" class="text-gray-400 text-sm">Seleccione metas...</span>
                        <span class="ml-auto text-gray-400 text-xs" x-text="open?'▴':'▾'"></span>
                    </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" wire:model="fecha_inicio" class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm focus:border-blue-500" />
                ...
                <div>
                    ...
                    <input type="date" wire:model="fecha_finalizacion" class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm focus:border-blue-500" />
