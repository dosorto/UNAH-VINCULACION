<div class="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
    ...
    @if (session('status'))
        <div class="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700 dark:border-emerald-900/60 dark:bg-emerald-950/40 dark:text-emerald-300">{{ session('status') }}</div>
    @endif
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                            @if ($selectedTipoPrograma)
                                {{ $workflowId ? 'Edita el flujo actual o reordena sus etapas.' : 'Este tipo todavia no tiene flujo. Puedes crear uno ahora.' }}
                            @else
                            ...
                            <div class="relative overflow-hidden rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
                                ...
                                <div class="flex items-center justify-between gap-3">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <button wire:click="removeStage({{ $index }})" class="text-sm font-medium text-rose-600">Quitar</button>
                                    </div>
                                </div>
