<div x-data="{ tab: 'pendientes' }" class="space-y-8 px-4 py-8 sm:px-6 lg:px-8">
    <section class="flex flex-col gap-4 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800 lg:flex-row lg:items-end lg:justify-between">
        <div>
            <p class="text-sm font-semibold uppercase tracking-[0.28em] text-emerald-700 dark:text-emerald-400">Revision institucional</p>
    ...
    <section class="rounded-3xl bg-white shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
        <div class="border-b border-slate-200 px-6 py-5 dark:border-slate-800">
            <div class="flex flex-wrap items-center gap-3">
                <button @click="tab = 'pendientes'" :class="tab === 'pendientes' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300'" class="rounded-full px-4 py-2 text-sm font-medium transition">
                    Pendientes
                    <span class="ml-2 rounded-full bg-white/20 px-2 py-0.5 text-xs">{{ $programasPendientes->count() }}</span>
                </button>
                <button @click="tab = 'proceso'" :class="tab === 'proceso' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300'" class="rounded-full px-4 py-2 text-sm font-medium transition">
                    Revisados en proceso
                    <span class="ml-2 rounded-full bg-white/20 px-2 py-0.5 text-xs">{{ $programasEnProceso->count() }}</span>
                </button>
                <button @click="tab = 'aprobados'" :class="tab === 'aprobados' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300'" class="rounded-full px-4 py-2 text-sm font-medium transition">
                    Aprobados
