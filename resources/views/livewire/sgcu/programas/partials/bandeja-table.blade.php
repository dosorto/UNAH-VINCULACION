                        ...
                        <div class="flex min-w-64 flex-col items-end gap-2">
                            @if (in_array($row->estado, ['ASIGNADO', 'PENDIENTE'], true))
                                ...
                                <div class="flex justify-end gap-2">
                                    <button type="button" x-on:click.prevent="confirmDialog('¿Aprobar esta etapa?').then((ok) => ok && $wire.approveRevision({{ $row->id }}))" class="rounded-full border border-emerald-300 px-3 py-1.5 text-xs font-semibold text-emerald-700 dark:border-emerald-800 dark:text-emerald-300">
                                        Aprobar
                                    </button>
                                    <button type="button" x-on:click.prevent="confirmDialog('¿Enviar el programa a subsanacion?').then((ok) => ok && $wire.rejectRevision({{ $row->id }}))" class="rounded-full border border-rose-300 px-3 py-1.5 text-xs font-semibold text-rose-700 dark:border-rose-800 dark:text-rose-300">
                                        Subsanar
                                    </button>
                                </div>
