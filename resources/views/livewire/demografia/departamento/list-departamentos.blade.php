<div>
    <div class="mb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
            ...
            <p class="text-zinc-500 dark:text-gray-400 font-medium text-sm">A continuación se muestra el listado de departamentos.</p>
        </div>
        <a href="{{ route('crearDepartamento') }}" wire:navigate
            ...
            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-100 dark:divide-gray-800">
                @forelse ($records as $record)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                        <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ $record->pais->nombre ?? '—' }}</td>
                        ...
                        <td class="px-4 py-3 text-right space-x-2">
                            ...
                            <button type="button" x-on:click.prevent="confirmDialog('¿Eliminar?', { type: 'danger' }).then((ok) => ok && $wire.delete({{ $record->id }}))" class="px-2.5 py-1 text-xs text-red-700 bg-red-50 hover:bg-red-100 dark:bg-red-900/30 dark:text-red-400 rounded-md">Eliminar</button>
                        </td>
                    </tr>
                @empty
    ...
    <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" wire:click.self="$set('editModal', false)">
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-lg p-6 mx-4">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Editar Departamento</h3>
            <div class="grid grid-cols-1 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">País *</label>
                    <select wire:model="edit_pais_id" class="w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm">
                        <option value="">Selecciona un país</option>
                        @foreach ($paises as $id => $nombre)
                            <option value="{{ $id }}">{{ $nombre }}</option>
                        @endforeach
                    </select>
                    @error('edit_pais_id') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
