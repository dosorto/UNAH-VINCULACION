...
class HistorialProyectoWorkflowStageResubmissionIntegrationTest extends TestCase
{
    ...
    public function test_subsanar_detecta_firma_rechazada_por_etapa_y_reenvia_por_el_camino_nuevo(): void
    {
        ...
        $this->assertCount(3, $firmasNuevoCiclo);
        $this->assertSame($context['etapas'][0]->id, $firmasNuevoCiclo[0]->flujo_aprobacion_etapa_id);
        $this->assertSame($context['etapas'][1]->id, $firmasNuevoCiclo[1]->flujo_aprobacion_etapa_id);
        $this->assertSame($context['etapas'][2]->id, $firmasNuevoCiclo[2]->flujo_aprobacion_etapa_id);
        $this->assertSame($firmaAnterior->empleado_id, $firmasNuevoCiclo[0]->empleado_id);
        $this->assertSame($firmaRechazada->empleado_id, $firmasNuevoCiclo[1]->empleado_id);
        $this->assertSame($firmaPosterior->empleado_id, $firmasNuevoCiclo[2]->empleado_id);
        ...
        $this->assertFalse($context['proyecto']->firmaEsActualEnFlujoPorEtapa($firmasNuevoCiclo[2]));
