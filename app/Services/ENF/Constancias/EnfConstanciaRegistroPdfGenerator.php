...
class EnfConstanciaRegistroPdfGenerator
{
    public function content(EnfConstanciaRegistro $constancia): string
    {
        $token = Crypt::decryptString((string) $constancia->token_cifrado);
        $url = route('enf.constancias.registro.verificar', ['token' => $token]);
        $qrPath = $this->temporaryQr($url);

        try {
            return Pdf::loadView('pdf.enf.constancias.registro', [
